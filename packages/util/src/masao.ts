...
import { param, format, playlog, load } from 'masao';
...
export function minimize(game: MasaoJSONFormat): MasaoJSONFormat {
  ...
  let advm2 =
    advm != null
      ...
      : void 0;
  advm2 = advm2 != null ? param.minimizeAdvancedData(advm2) : void 0;

  // paramsから余計なものを除去
  let params2 = filterParams(params, (key, _) => {
...
// formatの変換
export function formatToMetadata(
  format: MasaoJSONFormat,
): Partial<GameEditableMetadata> {
  const { metadata } = format;
  const title =
    metadata != null && 'string' === typeof metadata.title
      ? metadata.title
      : '';
  return {
    title,
...
export function formatToGame(
  format: MasaoJSONFormat,
  resources?: Array<Resource>,
): Game {
  const { params, version, script } = format;
