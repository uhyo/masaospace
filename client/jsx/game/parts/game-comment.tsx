...
import { Session, Comment } from '../../data';
...
export default class GameComment extends React.Component<
  IPropGameComment,
  IStateGameComment
> {
  ...
  render() {
    //プレイログを選択
    const {
      props: { game, playlogs, onPlay, config, session },
      state: { loading, playlog, comments },
    } = this;
    ...
    if (session.loggedin === false) {
      ...
      if (playlogs.length > 0) {
        ...
        if (playlog == null) {
          ps = (
            <p>
              コメントと一緒にプレイログを投稿するには、以下から選択してください。
            </p>
          );
        } else {
          ...
          ps = (
            <p>
              <span className="clickable" onClick={clk}>
                プレイログの投稿を取り消す
              </span>
            </p>
          );
        ...
        playlogArea = (
          <div className="game-play-comments-playlog">
            {ps}
            <PlaylogList
              playlogs={playlogs}
              playString="選択"
      ...
      commentForm = (
        <CommentForm
          game={game}
          config={config}
          playlog={selectedPlaylog && selectedPlaylog.buffer}
    ...
    return (
      <section className="game-play-comments">
        <h1>コメント</h1>
    ...
    return (
      <div className="game-play-comments-select">
        {['new', 'score'].map((m: 'new' | 'score') => {
          ...
          return (
            <div key={m} className={cl} onClick={this.handleModeChange(m)}>
              {m === 'new' ? '新着順' : m === 'score' ? 'スコア順' : null}
            </div>
          );
        })}
      </div>
