...
export default class Play extends React.Component<IPropPlay, IStatePlay>{
    ...
    render(){
        ...
        //プレイログ
        const playlogs = playlog_score!=null ?
        ...
        if(playlogs.length>0 || playlog_playing_data!=null){
            ...
            if(playlog_score!=null || playlog_clear!=null){
                const plist = playlog_score!=null ?
        ...
        if(game.script == null){
            ...
            //TODO jsファイルのやつは？
            gameView = <GameView game={game} audio_enabled={audio_switch} playlogCallback={playlogCallback} playlog={playlog_playing_data}/>;
        }else{
            gameView = <SafeGameView game={game} audio_enabled={audio_switch} config={this.props.config} session={session}/>;
        }
        ...
        return <section>
            ...
            <GameTools config={this.props.config} game={game} metadata={metadata} audio={audio_switch} onAudioChange={handleAudioChange}/>
            <GameComment game={metadata.id} playlogs={playlogs} config={this.props.config} session={session} onPlay={this.handlePlay.bind(this)}/>
        </section>;
    }
    //ゲームからplaylogが来たので
    ...
    render(){
        ...
        if(state.code===true){
            code=<div className="game-play-tools-code">
                ...
                <pre><code>{
                    `<iframe src="${config.service.url}embed/${metadata.id}" width="514" height="434" style="border:none"></iframe>`
                }</code></pre>
            </div>;
        }
        //audio
        let au=null;
        if(game.params.se_switch==="2" && game.params.fx_bgm_switch==="2" || game.version!=="fx"){
            //効果音はいらなそう
            ...
            if(audio===true){
                au=<span className="icon icon-sound clickable" title="音声がONになっています。" onClick={audioClick}/>
            }else{
                au=<span className="icon icon-sound-off clickable" title="音声がOFFになっています。" onClick={audioClick}/>
            }
        }
        //social
