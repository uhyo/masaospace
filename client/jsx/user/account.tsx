...
class ProfileForm extends React.Component<IPropProfileForm, IStateProfileForm> {
  ...
  render() {
    const {
      props: { config, userdata },
      state: { name, url, profile, icon, modified },
    } = this;
...
class IconEdit extends React.Component<IPropIconEdit, IStateIconEdit> {
  ...
  render() {
    const {
      props: { config, userdata, icon, onChange },
      state: { edit },
    } = this;
    let filelist = null;
    if (edit === true) {
    ...
    return (
      <div className="user-account-profile-icon-wrapper">
        <div className="user-account-profile-icon-menu">
          <div
            className="user-account-profile-icon-preview"
          ...
          <div>
            ...
            {edit ? (
              <p>アイコンを変更したら、下の保存ボタンを押してください。</p>
            ) : (
              <p>アイコンをクリックして変更できます。</p>
            )}
          </div>
        </div>
        {filelist}
...
class ChangePasswordForm extends React.Component<
  IPropChangePasswordForm,
  IStateChangePasswordForm
> {
      ...
      if (name === 'current' || name === 'password') {
        ...
        if (t.value.length < this.props.config.user.password.minLength) {
          ...
          if ((t.validity as any).tooShort !== true) {
            ...
            t.setCustomValidity(
              'パスワードが短すぎます。最低' +
                this.props.config.user.password.minLength +
                '文字入力してください。',
            );
          }
        } else {
...
class MailForm extends React.Component<IPropMailForm, IStateMailForm> {
  ...
  render() {
    ...
    return (
      <div>
        <p>
          新しいメールアドレスを入力してください。確認用のメールが送信されます。
        </p>
        <form className="form" onSubmit={this.handleSubmit.bind(this)}>
          <p>
            <label className="form-row">
      ...
      <div className="icon-menu">
        {['edit', 'del'].map((md: 'edit' | 'del') => {
          ...
          return (
            <div
              key={md}
              className={'icon-menu-item' + selected}
              onClick={onClick}
            >
...
class FileDelForm extends React.Component<IPropFileDelForm, IStateFileDelForm> {
  ...
  render() {
    const {
      props: { config, session, usage },
      state: { mode, alternativeFile },
    } = this;
    if (mode === 'start') {
      return (
        <div>
      ...
      return (
        <div>
          <p>
            このファイルは
            <b>{this.state.used}</b>
            件の正男で使用されています。ファイルを削除するには、他のファイルで置き換えてください。どのファイルで置き換えますか？
          </p>
          <FileList
            config={config}
            query={query}
            currentFile={alternativeFile || void 0}
            onChange={handleChange}
            useDefault
            usePreviewLink
          />
...
class SeriesPage extends React.Component<IPropSeriesPage, IStateSeriesPage> {
  ...
  render() {
    ...
    if (selected !== -1) {
      ...
      newSeries = (
        <div
          className="vertical-menu-item vertical-menu-selectable"
          onClick={newHandler}
        >
          新しいシリーズを作成...
        </div>
      ...
      newSeries = (
        <div className="vertical-menu-item">
          ...
          <SeriesForm
            config={config}
            owner={session.user}
            onSubmit={this.newSubmitHandler.bind(this)}
          />
        </div>
      );
    }
    ...
    //シリーズをソート
    return (
      <div className="user-account-series">
        <div className="vertical-menu">
          {series.map(({ id, name, games }, i) => {
            ...
            return (
              <div className={c} key={id} onClick={selectHandler}>
                {name} ({games.length})
              </div>
            );
          })}
...
class SeriesForm extends React.Component<IPropSeriesForm, IStateSeriesForm> {
  ...
  componentDidMount() {
    const { useGamesEdit, id } = this.props;
  ...
  render() {
    const {
      props: { name, description, owner, saveButton, useGamesEdit },
      state: { loading, games },
    } = this;
    ...
    if (loading === true) {
      ...
      gamesArea = (
        <section className="user-account-seriesform-gamelist">
          <h1 className="legend">正男の一覧</h1>
          <p>
            現在
            <b>{games.length}件</b>
            の正男が追加されています。正男はドラッグ&amp;ドロップで並び替えができます。
          </p>
          <GameList
...
class GameList extends React.Component<IPropGameList, IStateGameList> {
  ...
  render() {
    const {
      props: { owner, games, onChange },
      state: { newMode, dragging, dragged, dragTarget },
    } = this;
    let newArea, nmo;
    ...
    if (newMode === false) {
      ...
      newArea = (
        <div className={c} onClick={addNewHandler} onMouseOver={nmo}>
          新しい正男を追加...
        </div>
      );
    } else {
      ...
      newArea = (
        <div className={c} onMouseOver={nmo}>
          <GameListSelector
            owner={owner}
            onSelect={this.gameSelectHandler.bind(this)}
    ...
    return (
      <div className="vertical-menu" onMouseUp={mouseup}>
        {games.map((obj, i) => {
          var mousemove;
          ...
          return (
            ...
            >
              <span>{obj.title}</span>
              <span
                className="user-account-gamelist-del"
                onClick={handleGameDel}
              >
                ✖
              </span>
            </div>
  ...
  protected handleMouseUp() {
    //ゲームを移動する
    const {
      props: { games, onChange },
      state: { dragTarget, dragged },
    } = this;
    if (dragged == null) {
      return;
    }
  ...
  protected gameSelectHandler(game: GameMetadata) {
    //新しいゲームがきた
    const { games, onChange } = this.props;
...
class GameListSelector extends React.Component<
  IPropGameListSelector,
  IStateGameListSelector
> {
  ...
  render() {
    ...
    return (
      <div>
        <p>
          シリーズに追加する正男を選択してください。
          <span className="clickable" onClick={closeHandler}>
            閉じる
          </span>
        </p>
        <div className="vertical-menu">
