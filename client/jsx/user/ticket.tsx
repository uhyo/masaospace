...
export default class Ticket extends React.Component<IPropTicket, IStateTicket>{
    ...
    render(){
        ...
        if(type === "setpassword"){
            return <section>
                <h1>パスワード設定</h1>
                <SetPassword screen_name={screen_name} ticket={ticket} config={config}/>
            </section>;
...
class SetPassword extends React.Component<IPropSetPassword, IStateSetPassword>{
            ...
            if(name==="password"){
                ...
                if(value.length < config.user.password.minLength){
                    ...
                    if((t.validity as any).tooShort!==true){
                        //自分でアレする
                        t.setCustomValidity("パスワードが短すぎます。最低"+config.user.password.minLength+"文字入力してください。");
                    }
                }else{
                    t.setCustomValidity("");
                }
    ...
    render(){
        ...
        return <form className="form" onSubmit={this.handleSubmit.bind(this)}>
            <p>ユーザー<b>{screen_name}</b>のパスワードを登録します。</p>
            <p>
                <label className="form-row">
                    <span>パスワード</span>
                    <input type="password" name="password" ref="password" minLength={config.password.minLength} maxLength={config.password.maxLength} onChange={handleChange} />
                </label>
            </p>
            <p>
                <label className="form-row">
                    <span>再入力</span>
                    <input type="password" name="password2" ref="password2" minLength={config.password.minLength} maxLength={config.password.maxLength} onChange={handleChange} />
                </label>
            </p>
...
class ResetPassword extends React.Component<IPropResetPassword, IStateResetPassword>{
    ...
    render(){
        ...
        let password = null;
        if(show_password){
            password = <p>ユーザーIDは<b>{screen_name}</b>, 新しいパスワードは<b>{newpassword}</b>です。</p>;
        }
        ...
        return <div>
            ...
            <p>パスワードを表示： <input type="checkbox" onChange={handleCheck} /></p>
            {password}
        </div>;
    }
}
