...
export default class FileSelect extends React.Component<IPropFileSelect, IStateFileSelect>{
    ...
    render(){
        ...
        return (
            <div className="fileselector">
                <div className="fileselector-dragarea" onDragEnter={handleDragEnter} onDragOver={this.handleDragOver} onDrop={this.handleDrop.bind(this)}>{
                    inn
                }</div>
            </div>
        );
    ...
    protected acceptedView(file: File){
        ...
        return (
            <div>
                <p>ファイル：{name}</p>
                {sizewarn}
                <p><span className="clickable" onClick={this.handleFileSelect.bind(this)}>ファイルを選択...</span></p>
            </div>
        );
    }
    protected acceptingView(){
        let accepts=null, limits=null;
        const {
            accept,
            maxsize,
        } = this.props;
