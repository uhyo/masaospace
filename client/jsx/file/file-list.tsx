...
import { stringKeys } from '../../scripts/keys';

import api from '../../actions/api';

import errorStore from '../../stores/error';

import { masao, ResourceKind } from '../data';
const { resourceKinds } = masao;
...
> {
  ...
  componentWillReceiveProps(nextProps: IPropFileList) {
    ...
    if (
      nextProps.forceLoad === true ||
      oldQuery.owner !== newQuery.owner ||
      oldQuery.usage !== newQuery.usage
    ) {
      this.setState(
        {
          file_upload: false,
  ...
  render() {
    const {
      props: { currentFile, useDefault, diskSpace, usePreviewLink, onChange },
      state: { query, loading, file_upload, files },
    } = this;
    if (loading === true) {
      return <Loading />;
    }
    ...
    if (diskSpace === true) {
      ...
      diskInfo = (
        <div className="file-list-info">
          <p>
            使用中: {bytes.format(disk)} /{' '}
            {bytes.format(this.props.config.filedata.diskSpace)}
          </p>
        </div>
      );
    }
            ...
            if (file === 'default') {
              ...
              return (
                <div key={i} className={className} onClick={handleClick}>
                  ...
                  <p className="file-list-file-description">
                    {file.description}
                  </p>
                  {previewLink}
                </div>
  ...
  protected fileUpload() {
    ...
    return (
      <FileUpload
        ...
        usage={this.state.query.usage || Object.keys(resourceKinds)[0]}
      />
    );
  }
