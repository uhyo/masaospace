//user action
import api from './api';
import { createAction, createAsyncAction } from '../scripts/reflux-util';
...
login.listen((params: { userid: string; password: string }) => {
  login.promise(
    api('/api/user/login', {
      user: params.userid,
...
update.listen(
  (params: { name: string; profile: string; icon: string; url: string }) => {
    update.promise(
      api('/api/user/update', {
        name: params.name,
