...
function localizeGame(game, domain){
    if(domain==null){
        domain="";
    }else{
        domain=`//${domain}`;
...
function getLastStage(params){
    var st = params.stage_max || masao.param.getDefaultValue("stage_max");
    return parseInt(st) || 1;
}
exports.getLastStage = getLastStage;
