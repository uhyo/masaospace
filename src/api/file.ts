...
export default class C {
  route(router: express.Router, c: Controller): void {
    ...
    router.post(
      ...
      (req, res) => {
        ...
        req
          ...
          .length(config.get('filedata.description.maxLength'));
    ...
    router.post('/edit', util.apim.useUser, (req, res) => {
      ...
      //まずファイルを取得
      c.file.getFiles(
        { id: req.body.id, owner: req.session!.user },
        (err, files) => {
          if (err || files == null) {
            res.json({
              error: String(err),
    ...
    router.post('/del', util.apim.useUser, (req, res) => {
      ...
      c.file.getFiles({ id: id, owner: req.session!.user }, (err, files) => {
        ...
        ('string' === typeof req.body.alternativeFile
          ? (callback: Cont) => {
              // alternativeFileが指定されている場合はこのファイルで置き換える
              (req.body.alternativeFile
                ? (callback: Cont) => {
                    //他のファイルで置き換える
                    c.file.getFiles(
                      {
