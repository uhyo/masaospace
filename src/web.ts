...
import { writeUserInfo, objectFromIterable } from './util';
...
import { masao, RouteHandler, SocialData } from '@uhyo/masaospace-util';
...
export class WebServer {
  ...
  init(c: Controller, callback: Cont): void {
    ...
    this.app = express();
    // set some methods
    this.app.request.validationErrorResponse = function(
    ...
    if (config.get('webserver.externalstatic') !== true) {
      ...
      this.app.use((req, res, next) => {
        if (
          /^\/(?:favicon\.ico|favicon-.*\.png|apple-touch-icon-.*\.png|android-chrome-.*\.png|mstile-.*\.png|manifest\.json|browserconfig\.xml)$/i.test(
            req.url,
          )
        ) {
          //favicon系の配信
  ...
  front(c: Controller): void {
    ...
    this.app.get('/embed/:id', (req, res) => {
      ...
      c.game.getGame(id, true, (err, obj) => {
        ...
        res.render('embed.ect', {
          constructorName:
            obj.game.version === '2.8' ? 'CanvasMasao_v28' : 'CanvasMasao',
    ...
    this.app.get('/sandbox/:id', (req, res) => {
      ...
      c.game.getGame(id, true, (err, obj) => {
        ...
        //正男をローカライズ
        var localGame = masao.localizeGame(
          obj.game,
          config.get('service.domain'),
        );
        ...
        .then(view => {
          ...
          var session =
            req.session!.user != null
              ? writeUserInfo(req.session! as Session)
              : null;
          var initialData = {
