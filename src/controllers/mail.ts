...
import { User, Mail, UserData } from '../data';
...
export default class MailController {
  ...
  userEntryMail(u: User<MUserData<UserData>>, ticket: string): void {
    ...
    this.send({
      ...
      text:
        ...
        'への登録が完了します。\n' +
        'URLの有効期間は発行されてから' +
        secondToString(config.get('ticket.life')) +
        'です。\n\n' +
        addr +
        '\n\n' +
        '----------\n' +
  ...
  resetPasswordMail(u: User<MUserData<UserData>>, ticket: string): void {
    ...
    this.send({
      ...
      text:
        ...
        'さん(' +
        d.screen_name +
        ')の新しいパスワードが発行され、古いパスワードは無効になります。\n' +
        'URLの有効期間は発行されてから' +
        secondToString(config.get('ticket.life')) +
        'です。\n\n' +
        addr +
  ...
  private getCollection(callback: Callback<db.Collection>): void {
    this.db.mongo.collection(
      config.get('mongodb.collection.mail'),
      (err, col) => {
        if (err) {
          logger.critical(err);
          callback(err, null);
